...
커뮤니티 글, 댓글, 프로필은 현재 브라우저 `localStorage`에 저장됩니다. 공통 게시판은 `weekend-korea-market-community-v2:common`, 종목 게시판은 `weekend-korea-market-community-v2:stock:TSLA`처럼 `boardId`별로 분리됩니다. 추후 `lib/communityStore.js`의 저장 함수만 Supabase 또는 Firebase 호출로 바꾸면 같은 UI 구조를 유지하면서 백엔드 저장소로 확장할 수 있습니다.

가격 데이터는 `app/api/quote/route.js`에서 Finnhub를 서버 사이드로 호출합니다. `FINNHUB_API_KEY`가 없거나 API 요청이 실패하면 `hooks/useLivePrices.js`가 기존 mock 가격을 4초 간격으로 자연스럽게 흔들어 보여줍니다.
